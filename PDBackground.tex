...
A state that operates on vectors and is itself vector valued naturally brings to mind a \todo{two-point} tensor such as the deformation gradient;
...
For a fuller \todo{more rigorous} definition and examples of the Fr\'echet derivative, see \cref{sec:frechet}.
...
\todo{I think ``First Law'' should be ``Stress Power''}
...
The first peridynamic models were all bond-based, and provide useful insight \todo{into what?  you might say useful insight into many complex material failure phenomenon despite\ldots} despite their limitations.
%
\begin{figure}[h]
  \centering
...
\end{equation}
%
For a small deformation gradient $\mathbf{F}$ \todo{I would say for small uniform displacements, i.e. $\nabla \mathbf{u} << 1$}, the stretch of bond $\boldsymbol{\xi}=\mathbf{q}-\mathbf{x}$ is
%
\begin{equation*}
...
This leaves only configurations such as $[1,2,1,2]$ and $[3,2,2,3]$, which we can indicate by $(\delta_{ik}\delta_{jl}+\delta_{il}\delta_{jk})$ \todo{What about $\delta_{ij}\delta{kl}$?}.
%
\begin{align}
...
\end{equation}
%
then the corresponding force state and peridynamic pressure are \todo{did you mean to have $\underline{t}^i$ here?  Also, why even define the peridynamic pressure if you don't use it?  I would use it in the constiutive model or omit it.}
%
\begin{equation}
...
Thus such a model is not possible in a bond-based framework without significant modification. \todo{If your looking to add a few pages the dissertation, you could include the recent work by Tupek on non-linear bond-strain models.  Just a thought.}
...
It may be necessary however to revisit the choice of model or implement some damage condition. \todo{The transition to the next chapter is a bit abrupt, you might consider adding a paragraph that explains how the models are different from the model you are going to develop in the next chapter.}
