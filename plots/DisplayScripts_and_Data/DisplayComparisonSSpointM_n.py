...
#Load PD plate data
PDfile1name = "./SS_moment_n100_h02_ext10_g001_l50/SS_moment_n100_h02_ext10_g001_l50_1_exp_7.npz"
...
data1mask = np.isnan(x01)
data1mask = np.zeros_like(x01)
...
#Load PD plate data
pdHorizon2 = 0.05
PDfile2name = "./SS_moment_n200_h04_ext10_g001_l50/SS_moment_n200_h04_ext10_g001_l50_1_exp_9.npz"
...
# #SS beam point load
# multiplier = gamma*(yieldstrain/thickness)/(6.0*(plate_length))
# left = analyticalX2*loada*(2*plate_length-3*loada+(loada**2)/plate_length)
# centerleft = analyticalX2*(plate_length-loada)*(plate_length**2-(plate_length-loada)**2-analyticalX2**2)/plate_length
# centerright=-loada*(plate_length-analyticalX2)*(loada**2+analyticalX2*(analyticalX2-2*plate_length))/plate_length
# right = (analyticalX2-plate_length)*((loada**3)/plate_length - loada*plate_length)

#SS beam point moment
multiplier = -gamma*(yieldstrain/thickness)/(6.0*(plate_length**2.0))
left = analyticalX2*(2*plate_length**2-6*loada*plate_length+3*loada**2)
centerleft = (analyticalX2**3+analyticalX2*(2*plate_length**2-6*loada*plate_length+3*loada**2))
centerright= (analyticalX2**3
    -3*plate_length*analyticalX2**2
    +analyticalX2*(2*plate_length**2+3*loada**2)-
    3*plate_length*loada**2)
right = (analyticalX2-plate_length)*(3*loada**2-plate_length**2)


analyticalZ2 = multiplier*np.select([analyticalX2<0.0,analyticalX2<loada,analyticalX2<plate_length,np.isfinite(analyticalX2)],[left,centerleft,centerright,right])

fig=plt.figure(1,figsize=(figureWidth,figureWidth*3.0/3.0))
plt.hold(True)
ax = fig.add_subplot(111)
ax1=ax.plot(analyticalX2,analyticalZ2,label="Analytical")
ax2=ax.plot(pdX01c,pdZ1c,ls="None", marker="^",markevery=(0,2),label="100 nodes, h=0.02")
ax3=ax.plot(pdX02c,pdZ2c,ls="None", marker="s",markevery=(2,4),label="200 nodes, h=0.02")

# ax = fig.add_subplot(211, projection='3d')
# ax.plot(analyticalX1,analyticalY1,analyticalZ1,ls="None", marker="o",label="Analytical")
plt.title('SS Beam With Point Moment')
...
ax.set_yticks(1.0e-5*np.linspace(-1.0,1.0,num=5))
...
if saving:
    ...
    fig.savefig("./writeup/plots/SSmoment_convergence_n.pgf")
plt.show()
